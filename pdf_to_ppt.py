...
for filename in os.listdir('source_files/'):
    ...
    trans = fitz.Matrix(zoom_x, zoom_y).preRotate(
        rotate)  # 旋转图片，如果图片方向不对的话改前面rotate的旋转角度
    ...
    if doc.pageCount > 1:  # 获取PDF的页数
        for pg in tqdm(range(doc.pageCount)):
            ...
            # 将其输入为相应的图片格式，可以为位图，也可以为矢量图
            pm.writeImage("./jpgs/%s/%s-%s.jpg" %
                          (new_full_name, new_full_name, pg))
    ...
    for index, page in enumerate(tqdm(pages)):
        ...
        slide.shapes.add_picture(
            jpg_file, left, top, height=prs.slide_height, width=prs.slide_width)
